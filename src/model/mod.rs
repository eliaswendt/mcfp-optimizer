...
impl Model {
    ...
    //     depth_steps: u64, 
    //     min_depth: u64,
    //     max_depth: u64 // maximum number of transfers to follow
    // ) -> Vec<(u64, Vec<EdgeIndex>)> {

    //     let mut result = Vec::new();

    //     // increase depth in 4 steps
    //     let depth_step = (max_depth - min_depth) / depth_steps;

    //     for i in 0..depth_steps+1 {

    //         let current_depth = min_depth + i * depth_step;

    //         println!("[iddfs()] trying with depth={}", current_depth);

    //         result = Self::all_paths_dfs_recursive(
    //             graph, 
    //             from, 
    //             to, 
    //             min_capacity, 
