...
pub fn optimize_overloaded_graph(
    ...
    for (edge_index, occupying_groups) in edges_2_groups.iter() {
        ...
        if timetable_edge.utilization() > timetable_edge.capacity() {
            println!(
                "Overcrowded edge found: capacity={}, utilization={}, groups={:?}",
                timetable_edge.capacity(),
                timetable_edge.utilization(),
                occupying_groups
            );
            overcrowded_edges.insert(
                *edge_index,
                timetable_edge.utilization() - timetable_edge.capacity(),
            );
        }
    }
...
fn edge_overcrowding(
    ...
    let timetable_edge = graph.edge_weight(edge_index).unwrap();
    let overcrowding =
        timetable_edge.utilization() as i64 - timetable_edge.capacity() as i64;
    if overcrowding > 0 {
